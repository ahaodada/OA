...
<script type="text/javascript">
    ...
    $("#btn_add").click(function () {
        window.location.href = "${ctx}/dept/addDept.action?flag=1";
    })
    ...
    $("#btn_delete").click(function () {
        ...
        if (ids.length < 1) {
            ...
            layer.confirm('确定删除这些部门吗?？', {
                ...
                //确定按钮的回调函数
                var url = "${ctx}/dept/removeDept.action";
                var params = {
                    flag: 2,
                    ids: ids
                }
                $.ajax({
                    ...
                    error: function () {
                        $("#table").bootstrapTable("refresh", {url: '${ctx}/deptJson.action'});
                        layer.closeAll('dialog');
                    }
                })
            })
    ...
    function deleteone(id) {
        layer.confirm('确定删除这个部门吗?？', {
            ...
            //确定按钮的回调函数
            window.location.href = "${ctx}/dept/removeDept.action?flag=1&id=" + id;
        })
//        },function () {
//			//取消按钮的回调函数
//        }
                ...
                columns: [
                    ...
                    {field: 'remark', title: '详细信息'},
                    {
                        ...
                        formatter: function (value, row, index) {
                            var element =
                                "<a  data-id='" + row.id + "' href='${ctx}/dept/updateDept.action?flag=1&id=" + row.id + "' class='btn btn-info btn-sm'>编辑</a> &nbsp;" +
