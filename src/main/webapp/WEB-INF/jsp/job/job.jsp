...
<script type="text/javascript">
    ...
    $("#btn_add").click(function () {
        window.location.href = "${ctx}/job/addJob.action?flag=1";
    })
    ...
    $("#btn_delete").click(function () {
        ...
        if (ids.length < 1) {
            ...
            layer.confirm('确定删除这些职位吗?？', {
                ...
                //确定按钮的回调函数
                var url = "${ctx}/job/removeJob.action";
                var params = {
                    flag: 2,
                    ids: ids
                }
                $.ajax({
                    ...
                    success:function(data){
                    	if(data=="success"){
                    		setTimeout(function(){
                    			$("#table").bootstrapTable("refresh", {url: '${ctx}/userJson.action'});
                    		},1000);
                    		layer.closeAll('dialog');
                    	}
                    },
                    error: function () {
                        $("#table").bootstrapTable("refresh", {url: '${ctx}/jobjson.action'});
                        layer.closeAll('dialog');
                    }
                })
            })
                ...
                columns: [
                    ...
                    {field: 'remanrk', title: '详细信息'},
                    {
                        ...
                        formatter: function (value, row, index) {
                            var element =
                                "<a  data-id='" + row.id + "' href='${ctx}/job/updateJob.action?flag=1&id=" + row.id + "' class='btn btn-info btn-sm'>编辑</a> &nbsp;" +
